...
int test_PowerIteration(int argc, char *argv[])
{
  ...
  EigenSolver::SP_solver solver = EigenSolver::Create(db);
  solver->set_operators(test_matrix_1(n));
  solver->solve(X, X0);
  cout << solver->eigenvalue() << endl;
  TEST(soft_equiv(solver->eigenvalue(), 9.09100627e-01, 1e-8));
  return 0;
}
...
{
  // Solve a generalized eigenvalue problem with SLEPc's GD.
  //
  int m = 10;
  ...
  db->put<std::string>("eigen_solver_type",          "slepc");
  db->put<std::string>("eigenvalue_solver_eps_type", "gd");
  db->put<double>("eigen_solver_tol",                1e-5);
  db->put<int>("eigen_solver_maxit",                 100);

  SlepcSolver solver(db);

  solver.set_operators(A, B);
  solver.set_preconditioner_matrix(C);
  solver.solve(X1, X0);
